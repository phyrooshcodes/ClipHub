    ...
    def _run(self):
        while True:
            if self._suspend_requested:
                logger.info("[YouTube Worker] Suspending browser for manual login...")
                self._close_context()
            ...
            try:
                ...
                if str(e) == "RESTART_HEADFUL":
                    logger.info("[YouTube Worker] Restarting browser in HEADFUL mode...")
                    self._close_context()
                else:
                    logger.error(f"[YouTube Worker] Upload processing error: {e}")
                    if upload_id:
                        with self._lock:
                            self.results[upload_id] = {
                                "status": "failed",
